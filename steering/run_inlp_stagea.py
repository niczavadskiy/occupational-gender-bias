    ...
    *,
    ...
    for layer in want_layers:
        ...
        if not include_random:
            continue
        # При явном --ranks random матчится по тем же рангам (иначе confirmatory
        # L15 k=8,16 раздувается до всех степеней двойки из Phase A).
        rand_ranks = ranks if ranks is not None else detail["random_control_ranks"]
        for k in rand_ranks:
            if k > detail["k_found"]:
                continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    if primary_axis == "auto":
        tgt = str(sub_meta.get("target", "gender_choice"))
        primary_axis = "slot" if "slot" in tgt else "gender"
    ...
    for idx, cfg in enumerate(configs, start=3):
        ...
        if primary_axis == "slot":
            metrics["primary"] = {
                "id": "mean_R_slot_family",
                "formula": "R_i = |mean_layouts d_slot|_before − |...|_after",
                **metrics["slot_axis"]["reduction"],
            }
        ...
        print(
            ...
            f"primary={primary_axis} {pr['mean']:+.4f}  "
        ...
        print(f"  Δθ_dev {metrics['theta_prob_axis']['reduction']['mean']:+.4f}")
        if hc:
            print(
    ...
    def _rank_key(r: dict):
        primary = r["mean_R_slot"] if primary_axis == "slot" else r["mean_R_gender"]
        # candidates first by primary desc, then layer/rank
