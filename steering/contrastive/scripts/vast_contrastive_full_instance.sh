#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Свежий Vast → clone → smoke → full contrastive Stage A (Qwen3.5-2B-Base) → pack.
#
#   export HF_TOKEN=hf_xxx
#   bash steering/contrastive/scripts/vast_contrastive_full_instance.sh
#
# Env: SKIP_SMOKE=1  SKIP_FULL=1  BRANCH TAG MODEL SCALE
# Default SCALE=2b. Для 4B: SCALE=4b MODEL=Qwen/Qwen3.5-4B-Base
# ---------------------------------------------------------------------------
set -euo pipefail
...
if [ "$SKIP_FULL" != "1" ]; then
  echo "=== [5] FULL (95 families × 47 configs) ==="
  PY="$PY" SMOKE=0 SCALE="$SCALE" TAG="$TAG" MODEL="$MODEL" REPO="$REPO" \
    bash steering/contrastive/scripts/vast_contrastive_stagea_and_pack.sh
fi

echo "=== ALL DONE ==="
ls -lh /workspace/contrastive_stage_a_*.tar.gz 2>/dev/null || true
echo "Скачай pack: /workspace/contrastive_stage_a_${TAG}.tar.gz"
