#!/usr/bin/env bash
# INLP Stage C report + pack for download.
set -euo pipefail
...
test -f steering/samples/inlp_stagec_sample_v1.json || {
  echo "MISSING inlp_stagec_sample_v1.json — git pull"
  exit 1
}

echo "=== INLP Stage C report [$TAG] ==="
...
OUT=/workspace/inlp_stage_c_${TAG}.tar.gz
tar -czf "$OUT" -C results/steering/inlp_stage_c "$TAG"
ls -lh "$OUT"
