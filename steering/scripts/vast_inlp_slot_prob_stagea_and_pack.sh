#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Classical slot_prob INLP on Vast: live Phase A + Stage A + pack.
#
#   export HF_TOKEN=hf_xxx
#   cd /workspace/occupational-gender-bias && git pull
#   bash steering/scripts/vast_inlp_slot_prob_stagea_and_pack.sh
#
...
TAG="${TAG:-inlp_slot_prob_a_v1}"
SUB_TAG="${SUB_TAG:-v1}"
LAYERS="${LAYERS:-20,21,22,23}"
RANKS="${RANKS:-4,8,16}"
ALPHAS="${ALPHAS:-1.0}"
SMOKE="${SMOKE:-0}"
...
SUB="$REPO/steering/subspaces/inlp_slot_prob_${SUB_TAG}.npz"
...
if [ "$SKIP_BUILD" != "1" ]; then
  echo "========== Phase A live INLP slot_prob [$SUB_TAG] =========="
...
echo "========== Stage A preference [$TAG] primary=slot =========="
...
tar -czf "$OUT" \
  ...
  "inlp_slot_prob_${SUB_TAG}.npz" "inlp_slot_prob_${SUB_TAG}.json"
ls -lh "$OUT"
echo "DONE $OUT"
echo "Primary R is slot (S). Pre-peak stack was R≈0.21; single-layer L16 alone ≈0.15."
