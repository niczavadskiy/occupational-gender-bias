#!/usr/bin/env bash
# 2B polarity-pooled INLP: Stage A → B → C for promale then profemale.
#
# Candidate layers = gender_prob peak L16 + L15 + L14 (matched XY peak_prepeak).
...
CONFIG_REL="steering/configs/inlp_polarity_pool_2b_peak_prepeak.yaml"
...
chmod +x steering/scripts/vast_inlp_polarity_pool_full_instance.sh
...
for sid in "${SET_LIST[@]}"; do
  ...
  [ -n "$sid" ] || continue
  echo ""
  echo "========== polarity set: $sid =========="

  echo "=== [$sid] build samples ==="
  ...
  pack="/workspace/inlp_polarity_pool_${sid}_stage_abc_2b.tar.gz"
  echo "=== [$sid] pack → $pack ==="
  tar -czf "$pack" \
    "steering/samples/inlp_polarity_pool_${sid}_v1.json" \
    "steering/samples/inlp_polarity_pool_${sid}_train_v1.json" \
    "steering/samples/inlp_polarity_pool_${sid}_val_v1.json" \
    "steering/samples/inlp_polarity_pool_${sid}_test_v1.json" \
    steering/subspaces/inlp_gender_prob_polarity_pool_${sid}_v1.npz \
    steering/subspaces/inlp_gender_prob_polarity_pool_${sid}_v1.json \
    results/steering/inlp_polarity_pool \
    || echo "WARN: partial pack for $sid"
  ls -lh "$pack" || true
done

echo "=== all polarity INLP sets done ==="
