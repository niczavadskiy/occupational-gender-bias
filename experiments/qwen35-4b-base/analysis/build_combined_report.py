...
SRC_H1 = ROOT / "results/v1_full_pos_shuffle/metrics_h1_v1/latest/summary.html"
...
SRC_H11_DIR = ROOT / "results/v1_full_pos_shuffle/probes/v1_rep_prob"
...
def fmt_ci_pp(lo: float | None, hi: float | None) -> str:
    if lo is None or hi is None:
        return "—"
    if not (math.isfinite(lo) and math.isfinite(hi)):
        return "—"
    return f"[{100.0 * lo:+.1f}, {100.0 * hi:+.1f}] п.п."


def mcnemar_effect_ci(b: int, c: int, n: int) -> tuple[float | None, float | None]:
    """Wald 95% CI for effect δ=(b−c)/n."""
    if n <= 0:
        return None, None
    delta = (b - c) / n
    var = ((b + c) - (b - c) ** 2 / n) / (n * n)
...
def render_h5_rates_table(rows: list[dict]) -> str:
    ...
    hdr = (
        ...
        "<th>k_abstain</th><th>P(man)</th><th>95% CI man</th>"
        "<th>P(woman)</th><th>P(abstain)</th></tr>"
    )
    body = []
    for d in rows:
        ev = h5_evidence_label(d["evidence"])
        man_ci = "—"
        if d.get("ci_man_low") is not None and d.get("ci_man_high") is not None:
            man_ci = f"[{d['ci_man_low']:.1f}%, {d['ci_man_high']:.1f}%]"
        body.append(
            f"<tr><td><code>{html.escape(ev)}</code></td>"
            f"<td>{d['n']}</td><td>{d['k_man']}</td><td>{d['k_woman']}</td>"
            f"<td>{d['k_abstain']}</td>"
            f"<td>{d['p_man']:.1f}%</td><td>{man_ci}</td>"
...
def compute_h5_abstain_mcnemar(items_path: Path) -> list[dict]:
    ...
    def mcnemar(a: str, b: str) -> dict:
        ...
        effect = (nb - nc) / n if n else float("nan")
        ci_lo, ci_hi = mcnemar_effect_ci(nb, nc, n)
        return {
            "a": a, "b": b, "n": n, "nb": nb, "nc": nc,
            "chi2": chi2, "p": p, "effect": effect,
...
def build_h5_section(h3_data: dict, tests: list[dict]) -> str:
    ...
    for t in tests:
        ...
        eff = t.get("effect")
        eff_s = f"{100.0 * eff:+.1f}" if eff is not None and math.isfinite(eff) else "—"
        ci_s = fmt_ci_pp(t.get("ci_low"), t.get("ci_high"))
        test_rows += (
            f"<tr><td class='mcn-key'>{a} → {b}</td>"
            f"<td>{t['n']}</td><td>{t['nb']}</td><td>{t['nc']}</td>"
            f"<td>{eff_s}</td><td>{ci_s}</td>"
            f"<td>{fmt_p(t['p'])}</td>"
  ...
  McNemar проверяет H₀: b = c (p-value в таблице). effect = (b−c)/n.</p>
  <table class="mcn">
    <thead><tr><th class="mcn-key">Сравнение (abstain)</th><th>n пар</th><th>b</th><th>c</th>
    <th>effect п.п.</th><th>95% CI</th>
  ...
  <p class="note"><b>CI:</b> 95% Wald для effect=(b−c)/n (α=0.05, z=1.96).
  Границы <b>[lo, hi]</b> — в таблице у каждого сравнения.</p>"""
    card = summary_card(
        hypothesis=(
        ...
        result=(
            "Подтверждается: добавление evidence (любого пола) <b>значимо снижает</b> долю "
            "«Воздержался» относительно <code>no_evidence</code> — "
            "<b>68.4%</b> → <b>33.1%</b> (man_evidence) / <b>16.5%</b> (woman_evidence); "
            "женская подсветка снижает воздержание сильнее."
        ),
...
<section id="h5">
  ...
  (man / woman / Воздержался) по условиям evidence. Значимость направленных сдвигов man↔woman —
  в секции H3 (paired McNemar); значимость сдвигов <b>воздержания</b> — в тесте ниже.</p>
  {test_block}
  <script>
  (function() {{
    var rows = {chart_data};
    ...
    new Chart(document.getElementById('chartH5Abstain'), {{
      ...
      data: {{
        ...
        datasets: [
          ...
          {{ label: 'P(Воздержался) %', data: rows.map(function(d) {{ return d.p_abstain; }}), backgroundColor: '#c45c3e' }},
        ],
      }},
      options: {{
        responsive: true, maintainAspectRatio: false,
        plugins: {{ title: {{ display: true, text: 'Rates по evidence_shift для with_abstain' }} }},
...
<section id="h11">
  ...
  <div class="interpret">
    ...
    <ul>
      <li><b>Модель:</b> <code>Qwen/Qwen3.5-4B-Base</code>; hidden states 33 слоёв
      (L0 = embedding, L1–L32 = transformer blocks), <code>d_model=2560</code>.</li>
  ...
  <p class="note"><b>CI:</b> точечные оценки accuracy/AUC; классический 95% CI не строится —
  контроль — shuffle baseline (~chance).</p>
...
<header class="top">
  ...
  <code>Qwen/Qwen3.5-4B-Base</code>). Содержимое перенесено из родных отчётов.</p>
</header>
...
<section id="intro">
  ...
  <code>Qwen/Qwen3.5-4B-Base</code> в задачах неоднозначного выбора профессии — как на
...
<div class="callout">
  ...
  <ul>
    <li><b>H1</b> — <code>v1_full_pos_shuffle</code> (metrics <code>h1_v1</code>).</li>
    <li><b>H3 / H5</b> — <code>highlight-h3-full</code> (metrics <code>h3_v1</code>).</li>
    <li><b>H11</b> — <code>v1_full_pos_shuffle</code> (probes <code>v1_rep_prob</code>, gender-ось).</li>
  </ul>
