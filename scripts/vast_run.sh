...
#   HF_TOKEN=hf_xxx GH_TOKEN=ghp_xxx \
...
cleanup() {
  if [ "$KEEP" = "1" ]; then
    echo "=== KEEP=1 — инстанс $IID ОСТАВЛЕН (удалить: vastai destroy instance $IID) ==="
  else
    echo "=== [destroy] удаляю инстанс $IID ==="
    vastai destroy instance "$IID" || echo "  WARN: destroy не прошёл — проверь вручную!"
  fi
}
trap cleanup EXIT INT TERM

echo "=== [3] жду готовности (running + ssh) ==="
SSH_HOST=""; SSH_PORT=""
for i in $(seq 1 60); do
  INFO="$(vastai show instance "$IID" --raw 2>/dev/null || true)"
  read -r ST SSH_HOST SSH_PORT <<<"$(printf '%s' "$INFO" | python3 -c "
$PYJSON
print(d.get('actual_status','?'), d.get('ssh_host') or '-', d.get('ssh_port') or '-')" 2>/dev/null || echo '? - -')"
  echo "  [$i] status=$ST ssh=$SSH_HOST:$SSH_PORT"
  [ "$ST" = "running" ] && [ "$SSH_HOST" != "-" ] && break
  sleep 10
done
[ "$SSH_HOST" = "-" ] && { echo "инстанс не дошёл до running — выходим (он будет удалён trap'ом)"; exit 1; }

SSHCMD="ssh -o StrictHostKeyChecking=accept-new -p $SSH_PORT root@$SSH_HOST"
echo "=== [4] инстанс готов ==="
echo "    SSH:  $SSHCMD"

if [ -n "$REMOTE_CMD" ]; then
  echo "=== [5] выполняю REMOTE_CMD на инстансе ==="
  $SSHCMD "cd /workspace 2>/dev/null; $REMOTE_CMD"
  echo "=== REMOTE_CMD завершён ==="
else
  echo "    REMOTE_CMD пуст — подключайся сам строкой выше."
  [ "$KEEP" != "1" ] && echo "    ⚠️ KEEP!=1: инстанс удалится при выходе скрипта. Для ручной работы запускай с KEEP=1."
fi
